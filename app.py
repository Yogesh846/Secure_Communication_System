...
def obfuscate_text(text):
    chars = "!@#$%^&*()"
    return "".join(random.choice(chars) for _ in text)
...
def send_message():
    ...
    try:
        ...
        if image_file and allowed_file(image_file.filename):
            print(f"Attempting to save file: {image_filename}")  # Debug
            image_path = os.path.join(app.config['UPLOAD_FOLDER'], image_filename)
            image_file.save(image_path)
            print(f"File saved to: {image_path}")  # Debug
            if os.path.exists(image_path):
                print("File exists after save")  # Debug
            else:
                print("ERROR: File not saved!")  # Debugs
            if content:
